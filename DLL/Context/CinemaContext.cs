...
namespace DLL.Context {
    public class CinemaContext : DbContext {
        public CinemaContext(DbContextOptions<CinemaContext> options) : base(options) { Database.EnsureCreated(); }
        ...
        private void UserConfigure(EntityTypeBuilder<User> builder) {
            ...
            builder.HasOne(x => x.Person).WithOne(x => x.User).HasForeignKey<User>(x => x.PersonId).IsRequired();
            builder.HasOne(x => x.LoginData).WithOne(x => x.User).HasForeignKey<User>(x => x.LoginDataId).IsRequired();
            builder.HasMany(x => x.ActionsData).WithOne(x => x.User);
        ...
        private void LoginDataConfigure(EntityTypeBuilder<LoginData> builder) {
            ...
            builder.HasOne(x => x.User).WithOne(x => x.LoginData).IsRequired();
            builder.HasData(new LoginData {
                Id = 1, Email = "albanekpiss@gmail.com", Login = "funnybored24", Password = "strongpassbymy24"
            });
        }
        ...
        private void ActionsDatasConfigure(EntityTypeBuilder<ActionsData> builder) {
            builder.HasOne(x => x.User).WithMany(x => x.ActionsData).IsRequired();
        ...
        private void SessionConfigure(EntityTypeBuilder<Session> builder) {
            ...
            builder.HasOne(x => x.Film).WithMany(x => x.Sessions);
            builder.HasMany(x => x.Seats).WithOne(x => x.Session).IsRequired();
        }
        private void FilmConfigure(EntityTypeBuilder<Film> builder) {
            ...
            builder.HasMany(x => x.Sessions).WithOne(x => x.Film);
        }
        private void SeatConfigure(EntityTypeBuilder<Seat> builder) {
            builder.Property(x => x.Number).IsRequired();
            builder.Property(x => x.Price).IsRequired();
            builder.Property(x => x.Status).IsRequired();
            builder.HasOne(x => x.Session).WithMany(x => x.Seats).IsRequired();
        }
    }
}